    ...
    F: Fn(&T2) -> Result<T1, DistributionError> + Clone + Send + Sync,
{
    ...
    fn sample(
        ...
        self.distribution.sample(theta, rng)
    }
}
...
pub trait ValuableDistribution: Distribution + Sized {
    ...
    fn value<T2, F>(
        self,
        value: F,
    ) -> ValuedDistribution<Self, Self::Value, Self::Condition, T2, F>
    where
        T2: Event,
...
where
    ...
    U: RandomVariable,
{
    fn value<T2, F>(self, value: F) -> ValuedDistribution<Self, Self::Value, Self::Condition, T2, F>
    where
        T2: Event,
        F: Fn(&T2) -> Result<Self::Value, DistributionError> + Clone + Send + Sync,
    {
        ValuedDistribution::<Self, Self::Value, Self::Condition, T2, F>::new(self, value)
    }
}
